//
//  ShloopKit.h
//  ShloopKit
//
//  Created by Bogdan on 02/10/14.
//  Copyright (c) 2014 Bogdan Raducan. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for ShloopKit.
FOUNDATION_EXPORT double ShloopKitVersionNumber;

//! Project version string for ShloopKit.
FOUNDATION_EXPORT const unsigned char ShloopKitVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <ShloopKit/PublicHeader.h>
#warning If project doesn't build/run because of swift-errors or linker error, you should comment/decomment this line and try to run.
//#import "ShloopKit-Swift.h"
#import <ShloopKit/UIImage+ImageEffects.h>